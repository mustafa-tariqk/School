...
public class Strategy extends Tour {
    ...
    protected Location nextMove() {
        switch (Tour.strategy) {
            case "Warnsdorff":
                pos = warnsdorffMove();
                break;
            default:
                throw new IllegalArgumentException("Invalid strategy: " + Tour.strategy);
        }
        return pos;
    }
    ...
    private Location warnsdorffMove() {
        Piece p = new Piece();
        ArrayList<Location> unvisited = p.nextMoves(Tour.curr);
